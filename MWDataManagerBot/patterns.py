...
_INSTORE_APPAREL_SUPPRESS_SNEAKERS_PATTERN = re.compile(
    ...
    r"\b(?:long|short)\s+sleeve\b|"
    r"\b\d{1,2}\s*/\s*4\s+zip\b|\b1/4\s+zip\b|\bquarter\s+zip\b|"
    r"\b(?:tote|totes|handbag|handbags|purse|purses|crossbody|satchel|mini\s+tote)\b|"
    r"\b(?:dress|blouse|skirt|romper|jumpsuit|swimsuit|bikini)\b"
    r")\b",
    re.IGNORECASE,
)
...
def instore_sneakers_bucket_active(text: str) -> bool:
    ...
    if instore_explicit_footwear_intent(raw):
        return True
    # Brand-only hit (e.g. Puma/Nike in a long-sleeve listing) without footwear nouns — not sneakers.
    return bool(instore_strong_footwear_only_intent(raw))
...
def matches_instore_theatre(text: str, where_location: str = "") -> bool:
    """True when the lead is theatre / cinema merch (canonical; patterns live in this module)."""
    if not text:
        return False
...
_INSTORE_CARDS_TCG_CONTEXT_PATTERN = re.compile(
    r"(?i)(?:"
    r"pokemon\s*(?:cards?|tcg|booster|etb|pack|box|center)|pok[eé]mon\s*!?\s*(?:tcg|cards?)|"
    r"magic\s*the\s*gathering|\bmtg\b|yugioh|yu[\s-]?gi[\s-]?oh|"
    r"one\s*piece\s*(?:card|tcg)|dragon\s*ball\s*(?:super\s*)?(?:tcg|card)|"
    r"flesh\s*and\s*blood|fab\s*tcg|lorcana|digimon\s*(?:card|tcg)|"
    r"\btcg\b|\bccg\b|booster\s*(?:pack|box)|elite\s*trainer\s*box|\betb\b|"
    r"\bslab\b|psa\s*\d+|bgs\s*\d+|cgc\s*\d+|graded\s*card|"
    r"rookie\s*card|case\s*break|sealed\s*(?:box|pack)|"
    r"topps\b|panini\b|\bprizm\b|\bmosaic\b|upper\s*deck|bowman\b|donruss\b"
    r")",
)
_INSTORE_CARDS_SUPPRESS_PATTERN = re.compile(
    r"(?i)(?:"
    r"watch\s*list|watchlist|"
    r"customer\s+service\s+and\s+give\s+them\s+sku|members\s+confirmed|"
    r"if you are near\b|"
    r"pre[- ]?order|"
    r"restock(?:ing)?\s+(?:tomorrow|tonight|today|soon)|"
    r"\b(?:cereal|cookies?|snack|chips|soda|frozen\s+meal|grocery)\b"
    r")",
)


def instore_cards_bucket_active(text: str) -> bool:
    """TCG/collectibles flip leads only — not bare @Pokemon pings, food, or watchlist stubs."""
    raw = str(text or "").strip()
    if not raw or _INSTORE_CARDS_SUPPRESS_PATTERN.search(raw):
        return False
    if not CARDS_PATTERN.search(raw):
        return False
    return bool(_INSTORE_CARDS_TCG_CONTEXT_PATTERN.search(raw))
