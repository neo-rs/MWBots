...
async def _get_or_create_webhook_url(*, dest_channel, reason: str, force: bool = False, webhook_name: str = "MWDiscumBot fetchall") -> str:
    try:
    ...
    try:
        nm = str(webhook_name or "MWDiscumBot fetchall").strip() or "MWDiscumBot fetchall"
        nm = nm[:80]
        wh = await dest_channel.create_webhook(name=nm, reason=str(reason or "MWDiscumBot fetchall webhook"))
...
async def send_via_webhook_or_bot(
    *,
    ...
    url = await _get_or_create_webhook_url(dest_channel=dest_channel, reason=reason, force=False, webhook_name="MWDiscumBot fetchall")
    if url:
